...
<?php
	...
	if($id!='')
	{
		$query1=mysql_query("select * from uas where email like('%$id%')");
...
<?php
	while($rec1=mysql_fetch_array($query1))
	{
		...
		$userid=$rec1[0];
		$email=$rec1[2];
		$query2=mysql_query("select * from uas where email like('%$id%')");
...
?>
		...
		<td bgcolor="#FFFFFF" style="padding-right:7;"><?php echo $email; ?></a>  </td>
		
		<td onMouseOver="serched_name_over1(<?php echo $uid;?>)" onMouseOut="serched_name_out1(<?php echo $uid;?>)" width="500" bgcolor="#FFFFFF" id="Name_bg1<?php echo $uid; ?>"> <a href="#" style=" text-decoration:none; text-transform:capitalize; color:#3B5998;" id="Name_font1<?php echo $uid;?>">  <?php echo $name; ?></a></td>
